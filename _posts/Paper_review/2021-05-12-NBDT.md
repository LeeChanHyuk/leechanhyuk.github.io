...
## Model - Decision tree configuration
 ...
 - 이 단계에서 leftnode는 $R^(D*K)$ 인 것을 보면 FC의 Weight의 개수가 DxK일 때 agglomerative clustering을 해서 K개의 class로 만드는 것 같다.
...
## Model - inference
 ...
 1. Seed oblique decision rule weights with neural networks weights.
    ...
    - 하지만 어차피 부모 노드는 자식 노드의 평균이므로, 그냥 Inner node 역시 바로 밑에 자식 노드 두 개의 평균을 취한다고 보면 되겠다.
...
## Experiments
 ...
 - NBDT는 Interpretability를 신경쓴 모델 중에서는 성능이 SOTA 수준이고 다른 Image classification model에 비해서도 높은 성능을 나타냈다.
...
## Appendix - C -  HIERARCHICAL SOFTMAX AND CONDITIONAL EXECUTION
